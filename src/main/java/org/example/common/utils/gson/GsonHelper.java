...
//    }
//    
// // Tạo một thực thể Gson dùng chung có khả năng xử lý LocalDateTime
...
public class GsonHelper {
    ...
    // CHỈ DÙNG 1 biến gson duy nhất, tích hợp sẵn Adapter
    public static final Gson gson = new GsonBuilder()
            .registerTypeAdapter(LocalDateTime.class, new LocalDateTimeAdapter())
            // Giữ cho JSON nằm trên 1 dòng để truyền qua Network không bị lỗi ngắt quãng
            .disableHtmlEscaping() 
            .create();
    ...
    /**
     * Đọc file JSON và chuyển thành Object
     */
    public static <T> T readJsonFile(String path, Class<T> clazz) {
        try (FileReader reader = new FileReader(path)) {
            return gson.fromJson(reader, clazz);
        } catch (IOException e) {
            log.error("Lỗi đọc file JSON: " + e.getMessage());
            return null;
        }
    }

    /**
     * Ghi Object xuống file JSON
     */
    public static boolean writeJsonFile(String path, Object data) {
        try (FileWriter writer = new FileWriter(path)) {
            gson.toJson(data, writer);
            ...
            log.error("Lỗi ghi file JSON: " + e.getMessage());
            return false;
        }
    }
}